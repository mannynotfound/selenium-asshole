...
class TwitterApi {
  ...
  clearAccount() {
    ...
    jsonfile.writeFile('./recipes/twitter-api/models/accounts-suspended.json', badAccts, {spaces: 2}, (err) => {
      if (err) {
        console.log('JSON FILE ERROR')
        console.error(err)
        process.exit()
      } else {
        jsonfile.writeFile('./recipes/twitter-api/models/main.json', this.model, {spaces: 2}, (err) => {
          if (err) {
            console.log('JSON FILE ERROR')
            console.error(err)
          } else {
